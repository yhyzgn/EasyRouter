package com.yhy.router.compiler;
...
public class AutowiredCompiler extends AbstractProcessor {
    ...
    private void parseAutowired(Set<? extends Element> elements) throws IOException {
        ...
        TypeMirror tmFragment = mEltUtils.getTypeElement(Constant.FRAGMENT).asType();
        TypeMirror tmFragmentX = mEltUtils.getTypeElement(Constant.FRAGMENT_X).asType();
        ...
        if (MapUtils.isNotEmpty(mTypeMap)) {
            for (Map.Entry<TypeElement, List<Element>> et : mTypeMap.entrySet()) {
                ...
                className = type.getSimpleName() + Constant.SUFFIX_AUTOWIRED;
                ...
                for (Element elt : fields) {
                    ...
                    if (elt.getModifiers().contains(Modifier.PRIVATE)) {
                        ...
                        // 获取字段
                        inject.addStatement(Constant.PRIVATE_FIELD_NAME + " = instance.getClass().getDeclaredField(\"" + fieldName + "\")");
                        // 设置私有字段可访问性
                        inject.addStatement(Constant.PRIVATE_FIELD_NAME + ".setAccessible(true)");
                        // 设置字段值
                        statement = Constant.PRIVATE_FIELD_NAME + ".set(instance, instance.";
                        ...
                        if (statement.startsWith(Constant.JSON_PARSER_NAME)) {
                            ...
                            inject.addStatement(Constant.PRIVATE_FIELD_NAME + ".set(instance, " + statement, argName, elt.asType(), argName);
                        } else {
                            // private Object
    ...
    private String buildStatement(boolean isActivity, String defValue, String statement, int type) {
        if (type == TypeKind.BOOLEAN.ordinal()) {
            ...
            // 需要Json解析
            statement = Constant.JSON_PARSER_NAME + ".fromJson(instance." + (isActivity ? "getIntent()." : "getArguments().") + (isActivity ? "getStringExtra($S)" : "getString($S)") + ", new $T<$T>(){}.getType())";
        }
        return statement;
    }
